...
- Configure SMTP on both apps so password resets, notifications, and alerts reach users reliably
- Disable open registration in Rocket.Chat — only invited users should be able to create accounts
- Switch Nextcloud background jobs from AJAX to system cron for reliable scheduling under load

```bash
# App Configuration
# Rocket.Chat — configure SMTP
# Admin → Settings → Email → SMTP
# SMTP Host: smtp.yourdomain.com  Port: 587  TLS: enabled

# Rocket.Chat — restrict open registration
# Admin → Settings → Accounts → Registration → set to "Disabled"

# Nextcloud — switch background jobs to system cron
# Admin → Basic Settings → Background Jobs → select "Cron"
